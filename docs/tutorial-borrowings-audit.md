...
核心结论：教程贴的**创作原则层**（§二 15 条创作原则 + 代入感六大支柱）几乎全部吸收；**雷点层**（18 landmines）完整借鉴；**小说设定层**（§三 16+ 条设定 + 24 个"严禁"）约三分之一进入通用铁律或 setting pack，其余属于特定小说设定不适用；**前言通用指令层**（12 条）几乎不适用（架构约束决定）；**角色层**（14 个）拆分重组到 7 个 Agent 的职责中。
...
**原文位置**：教程贴 L5
**相关架构决策**：`docs/superpowers/specs/2026-05-09-blackboard-novel-pipeline-design.md` L111-114
...
#### 3.12 创作自检 Checklist
**状态**：🟡 部分借鉴 — 教程贴的 4 条自检（时间事件、逻辑闭环、细节填充、对照设定）被分散吸收：
...
**落点**：上述分散在 `rules/24-iron-laws.md`、`src/agents/evaluator.py`
**差异**：教程贴是 Generator 生成前自检，系统是 Evaluator 事后检查（更可靠，因为「自己评自己乐观」是 Anthropic 踩过的坑）
...
- **书名简介**：🔴 未借鉴 — 系统不生成书名和简介，这部分是作品包装，当前 3 章 MVP 不涉及
...
| 16 | 作品包装缺乏吸引力 | landmine_16 | 🟡 部分（书名简介部分不适用） |
...
| 类别 | 代表条目 | 原因 |
|---|---|---|
| 联网搜索 | 前言 12 条中 4 条涉及搜索 | 架构约束：Agent 无搜索能力，setting pack 静态注入 |
| LLM 通用指令 | 语气规范/深入解析/Markdown 格式等 | 不适用于小说写作 Agent 的 prompt 系统 |
| 作品包装 | 书名、简介生成 | MVP 不覆盖发布流程 |
| 领域专家角色 | 股票/军火/雇佣兵大师 | 3 章 MVP 未涉及，且架构选择了通用 Agent + 题材注入而非专用专家 |
...
1. **联网搜索**：教程贴最强调的能力（12 条通用指令中有 4 条涉及搜索）。系统的第一个架构决策就是不联网——因为实时搜索引擎流不可靠、不可复现、海外爬取香港历史资料尤其困难。用 setting pack 预载替代。

2. **多种领域专家角色（军火/股票/雇佣兵）**：教程贴拟定了 14 个角色，其中 5 个是领域专家（L95-103）。系统只用 7 个 Agent（5 主+2 审计），不设领域专家——因为 setting pack 的 era.md 预载了足够的事实，且 MVP 3 章未涉及这些领域的深度剧情。

3. **写作练习与教学示例**：教程贴大量内容（如避免流水账的练习、开篇错误分析）本质是教学材料。系统是对手，不是老师——不需要教 LLM 怎么写，而是强行让写入规则的文字被遵守。

4. **书名/简介/发布流程**：教程贴讨论了书名测试、简介写法、番茄平台发布策略。系统是写作 pipeline，不涉及平台发布。

5. **「照搬百科」禁令**：教程贴说「严禁照搬百科」，但在系统中，setting pack 的 era.md 实际上就是「百科全书」——区别在于它被设计成供 Generator 在写作时内化吸收，而不是直接复制粘贴到正文。系统通过 iron_law_18（五感代入）和 writing-style-core（Show-Don't-Tell）间接实现了这个约束。
...
4. **严禁列表 → 铁律表格**：教程贴末尾的 24 条「严禁xxx」→ 24 条编号化的 iron_law
5. **生成前自检 → 事后独立审计**：教程贴的「创作自检 Checklist」→ 系统中的 Evaluator + AISlopGuard + CharacterGuard 独立审计

核心洞察：教程贴是一个作者与 AI 对话的工作流；系统是把这段对话中的**规则内容**提炼出来，变成多个独立 Agent 之间通过文件通信的**执行系统**。教程贴的「角色」和系统的「Agent」之间不是一一对应关系——大部分角色被拆解为检查规则注入到 Evaluator/Guard 中，少数（写手、责编）保留了独立 Agent 形态。
