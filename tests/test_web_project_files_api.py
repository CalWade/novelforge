...
def test_put_roundtrip_preserves_progress(client, isolated_project):
    """PUT must re-seed state/ BUT preserve progress.completed_chapters.

    Uses `isolated_project` fixture so the real gangster-hk-1983-linjiayao/
    on disk is never touched by this test.
    """
    pid = isolated_project

    # Read original content (the isolated project's project.yaml)
    ...
    assert new_progress["current_chapter"] == 7, (
        "progress was RESET by re-seed — preserve_progress=True broken"
    )
