...
任务：**同框架换核心设定** 地创造一个**新奇、不落俗套**的小说 preset。

具体要求：

## 1. 保留共性戏剧框架
找这 N 本的 **共同戏剧 DNA**（不是设定，是结构）：
- 末世感 / 系统机制 / 多角色群像 / 主角秘密身份……
- 情节单元的起承转合节奏
- 爽点配方、钩子配方、信息释放节奏

## 2. 原创核心设定
但**所有具体设定必须原创**，不得出现：
- 任何 N 本素材里的专属角色名、地名、物品名、组织名、技能名
- 任何 N 本素材的核心世界观公理（如"游戏开测"、"虫潮"、"神秘公司派员")

正面建议：
- 从日常生活中找一个"被忽视但诡异"的锚点当世界观核（如：所有人的梦境开始被流量化交易 / 语言有了物理重量 / 记忆可以被截断成碎片出售）
- 避开当下小说市场最俗滥的切入（"系统降临"、"丧尸围城"、"多元宇宙维护员"都算俗）

## 3. 融合写作风格
把 N 本的写作风格取**平均值 + 差异化优点**：
- 节奏 / 句式 / POV：取共性（大部分本都用同一种 → 保留）
- 钩子款式：取最犀利的一款作为主力
- 叙述视角：明确选一种
- 明确标明本 preset 的"口味"：是冷硬硬核风、是荒诞幽默风、还是抒情哲思风

## 4. 输出三个文件的内容（严格 JSON 格式）

按下列 schema 输出 JSON，不加任何额外解释：
...
```json
{
  ...
  "era_md": "<完整的 era.md 内容，Markdown 格式，至少 800 字>",
...
## 5. era.md 必须包含
- 世界观**核心反常**（一条，最新奇的那个设定）
- 世界发生了什么（历史锚点 / 当前状态 / 未来威胁）
- 社会结构 / 权力格局 / 主要阵营
- 主角的特殊身份 + 与世界反常的关系
- 3-5 个新创的地名、2-3 个新创的概念名词、若干新创的术语

## 6. writing-style-extra.md 必须包含
- 叙述视角和 POV 策略（明确到 单 POV / 双 POV 交替 / 多 POV 群像 / 全知）
- 3-5 条具体的"动笔时必须记住的招式"（不是'冷笔'这种抽象，是"对白占 40%，动作描写必须带声音"这种可操作）
- 典型的章首钩子 + 章末钩子样板各 2 条
- 明确列出"禁用的风格"（至少 3 条）

## 7. iron-laws-extra.md 必须包含
- **第 1 条必须是**："不得直接使用 N 本素材里的专属名词。下列是禁区：..."（列出源小说的核心设定关键词作为禁区）
- 其余 4-6 条是本题材的硬约束（如"物价必须贴合新世界观的时代"、"主角的金手指能力不得超过已设上限"）
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--dna-out", type=str, default="docs/novel_dna",
        help="DNA 卡归档目录（相对仓库根），默认 docs/novel_dna",
    )
    ...
    if args.dry_run:
        print(f"\n--dry-run: stopped after Stage 1. DNA cards in {dna_dir}/")
        return 0
