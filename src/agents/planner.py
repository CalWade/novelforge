...
class Planner(BaseAgent):
    ...
    def _build_prompts(self, bb: Blackboard, *, chapter: int, **_):
        ...
        system = (
            ...
            "    - `numeric_drift_risk`：本章可能在生活/感官层堆砌科学化数字的风险\n"
            "      （亚秒精度如『0.3 秒』、毫米级生理指标如『眨眼 0.6 毫米』、\n"
            "      百分比生理变化如『侵蚀速度 10.2%』、赫兹级感官频率等）。\n"
            "      **数字量化只用于灰烬契书系统层数据**（条款数 / 担保链节点 / 灰烬点 /\n"
            "      违约倒计时整秒），不得给生理感官标科学论文级精度。\n"
            "      **scene.sensory_prompts 严禁出现亚秒精度或毫米级生理指标**——\n"
            "      Generator 看到 plan 里这种数字会照搬，导致全章数字泛滥。\n"
        ...
        user = (
            ...
            '    "numeric_drift_risk": "<本章是否会堆砌生活/感官层科学化数字（亚秒精度/毫米/赫兹/生理百分比）的风险；无风险时写 \'数字仅用系统层（条款/担保链/灰烬点/整秒倒计时）\' 作为凭证>",\n'
