...
Design principle: no agent touches files directly. Every read and write goes
through this module so we can (a) enforce atomic writes, (b) keep the path
conventions in one place, and (c) later swap the backend (e.g., to a DB) in
one spot without touching agent logic.

All paths passed in are RELATIVE to the state directory unless marked absolute.
"""
